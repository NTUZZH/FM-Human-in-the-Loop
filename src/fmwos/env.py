...
class DispatchEnv:
    ...
    # ------------------------------------------------------------------ #
    # Supervised path: run a full episode with an in-loop supervisor      #
    # (Paper Y3 HITL).  Flag-gated: with supervisor=None the executed     #
    # code path is behaviourally IDENTICAL to run_policy (anchor E0).     #
    # ------------------------------------------------------------------ #
    def run_supervised(self, decider, supervisor=None, method: str = "rl",
                       seed: int = 0):
        """Run one episode with a decider that returns ``(job, margin)``.

        Parameters
        ----------
        decider : callable(queue, t, rng) -> (job, margin)
            Chooses a work order from the current trade's queue and reports a
            confidence margin (used only by the supervisor's review mechanism).
        supervisor : object or None
            If None, the decider's pick is executed unchanged and this reduces
            EXACTLY to ``run_policy`` (same driver, same picks, byte-identical
            schedule).  If given, its ``review(decider_pick, candidates, t,
            margin) -> (executed_pick, log_entry)`` may override the pick.
        Returns ``(schedule, log)`` where ``log`` is the per-decision override
        log (empty when supervisor is None).

        NOTE (v1 order-only override).  The env chooses only the ORDER at a
        decision event; the technician is the deterministic smallest-id idle
        one (``_driver`` line ~192).  A supervisor "preferred (technician,
        order) pick" therefore reduces to a preferred ORDER at that event, and
        the override replaces the executed order only.
        """
        t0 = time.perf_counter()
        self._reset_state()
        rng = random.Random(seed)
        ...
        try:
            next(gen)
            while True:
                q = self.queue[self._cur_trade]
                now = self._cur_now
                job, margin = decider(q, now, rng)
                if supervisor is not None:
                    candidates = list(q)
                    job, entry = supervisor.review(job, candidates, now, margin)
                    log.append(entry)
                gen.send(job)
        except StopIteration:
            pass
        return self._build_schedule(method, time.perf_counter() - t0, seed), log
